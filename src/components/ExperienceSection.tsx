...
export const ExperienceSection: React.FC = () => {
  ...
  return (
    <section id="experience" className="bg-[#121620] border border-[#1e2433] rounded-2xl sm:rounded-3xl p-4 sm:p-7 md:p-9 shadow-xl">
      {/* Section Header */}
        ...
        {EXPERIENCES.map((exp) => {
          ...
          return (
            <motion.div
              ...
              className={`bg-[#181d2a] border transition-all duration-300 rounded-xl sm:rounded-2xl p-4 sm:p-6 relative overflow-hidden shadow-md ${
            ...
            >
              ...
              {exp.status === 'active' && (
                <div className="absolute top-0 right-0">
                  <div className="bg-emerald-500/10 text-emerald-400 text-[9px] sm:text-[10px] font-mono font-bold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-bl-xl border-l border-b border-emerald-500/30 flex items-center gap-1.5">
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-[#121620] border border-[#272f42] flex items-center justify-center ${theme.activeText} shadow-sm shrink-0 mt-0.5`}>
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                        ...
                        {exp.slaMetrics && (
                          <span className="text-[9px] sm:text-[10px] font-mono text-slate-400 bg-[#121620] px-2 py-0.5 rounded border border-[#272f42] hidden xs:inline">
                            SLA: {exp.slaMetrics}
                          </span>
                        )}
                      </div>

                      <h3 className="text-base sm:text-xl font-bold text-slate-100 mt-1 flex flex-wrap items-center gap-1 sm:gap-2">
                        <span>{exp.company}</span>
                        <span className="text-slate-400 font-normal text-xs sm:text-base">— {exp.role}</span>
                      </h3>

                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-[11px] sm:text-xs font-mono text-slate-400">
              ...
              {/* Expandable details */}
              {isExpanded && (
                <div className="mt-4 sm:mt-5 pt-4 sm:pt-5 border-t border-[#1e2433] space-y-3 sm:space-y-4 animate-fadeIn">
                  <div>
                    <h4 className="text-[11px] sm:text-xs font-mono uppercase text-slate-300 font-bold tracking-wider mb-2 flex items-center gap-1.5">
                  ...
                  <div className="pt-1">
                    <div className="text-[10px] sm:text-[11px] font-mono uppercase text-slate-400 font-medium mb-1.5">
