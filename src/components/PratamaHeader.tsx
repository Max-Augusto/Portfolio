...
export const PratamaHeader: React.FC<PratamaHeaderProps> = ({ 
  ...
  return (
    <header className="w-full max-w-7xl mx-auto px-3 sm:px-6 pt-3 sm:pt-6 pb-2">
      ...
      <div className={`${theme.bgCard} border ${theme.borderCard} rounded-2xl sm:rounded-3xl p-3.5 sm:p-7 relative overflow-hidden shadow-2xl transition-all duration-300 w-full`}>
        
        {/* Top Console Bar */}
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between pb-3 sm:pb-5 border-b ${theme.borderCard} mb-4 sm:mb-6 gap-2.5 sm:gap-3`}>
          ...
          <div className="flex items-center justify-between sm:justify-start gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <div className={`flex items-center gap-1.5 sm:gap-2 px-2.5 py-1 rounded-full ${theme.bgSubCard} border ${theme.borderSubCard} text-[10px] sm:text-[11px] font-mono ${theme.textSecondary} shadow-xs`}>
              ...
              <span className="font-semibold text-emerald-500 truncate">{t('header.operator_status')}</span>
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center justify-between sm:justify-end gap-1.5 sm:gap-2 w-full sm:w-auto">
            
            {/* Group 1: Language & Theme Mode */}
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                onClick={toggleMode}
                className={`p-1.5 sm:p-2 rounded-xl ${theme.bgSubCard} hover:bg-slate-200 dark:hover:bg-[#202738] border ${theme.borderSubCard} ${theme.textSecondary} transition-all cursor-pointer shadow-xs min-h-[32px] shrink-0 flex items-center justify-center`}
            ...
            {/* Group 2: Swatches & Command Palette */}
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              {onOpenCommandPalette && (
                <button
                  ...
                  title="Abrir Busca / Command Palette (Ctrl+K ou ⌘K)"
                ...
                >
                  ...
                  <span className="text-[11px] sm:text-xs">{t('header.search')}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8 items-start lg:items-center">
          ...
          <div className="lg:col-span-5 flex items-start sm:items-center gap-3.5 sm:gap-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className={`text-xs sm:text-sm ${theme.textMuted} font-normal mt-0.5 leading-snug`}>
                {t('header.role')}
              </p>
          ...
          <div className={`lg:col-span-3 ${theme.bgSubCard} p-3 sm:p-3.5 rounded-xl lg:bg-transparent lg:p-0 lg:rounded-none lg:border-l ${theme.borderCard} lg:pl-6 flex flex-col gap-2 font-mono text-xs ${theme.textMuted}`}>
            <div className="text-[10px] sm:text-[11px] uppercase tracking-wider text-slate-500 font-semibold mb-0.5">
              SYSTEM STATUS
            </div>
            
            <div className="flex items-center justify-between sm:justify-start gap-2">
              <span className="text-slate-500 text-[11px] sm:text-xs shrink-0">status:</span>
              <button 
              ...
              >
                ...
                <span className={`text-[11px] sm:text-xs font-semibold ${isAvailable ? 'text-emerald-500' : 'text-amber-500'}`}>
            ...
            <div className="flex items-center justify-between sm:justify-start gap-2">
              <span className="text-slate-500 text-[11px] sm:text-xs shrink-0">local/tz:</span>
              <span className={`${theme.textSecondary} font-medium text-[11px] sm:text-xs`}>
                Betim / BH {currentTime ? `(${currentTime})` : ''}
              </span>
            </div>

            <div className="flex items-center justify-between sm:justify-start gap-2">
              <span className="text-slate-500 text-[11px] sm:text-xs shrink-0">resposta:</span>
              <span className="text-emerald-400 font-semibold text-[11px] sm:text-xs">&lt;24h SLA</span>
            </div>
          </div>
          ...
          <div className={`lg:col-span-4 flex flex-col sm:flex-row lg:flex-col items-stretch sm:items-center lg:items-end justify-between gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 ${theme.borderCard}`}>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-2 w-full lg:w-auto">
              ...
              >
                ...
                <span className="truncate max-w-[140px] xs:max-w-[180px] text-left">{personalInfo.email}</span>
              ...
              <div className="flex items-center gap-1.5 text-slate-400 text-sm shrink-0">
                <a
                  ...
                  className={`w-9 h-9 rounded-xl ${theme.bgSubCard} hover:${theme.textPrimary} border ${theme.borderSubCard} transition-colors flex items-center justify-center`}
                  title="GitHub"
                >
        ...
        {onSelectSection && (
          <div className={`mt-4 sm:mt-5 pt-3 sm:pt-4 border-t ${theme.borderCard}`}>
            ...
            {/* Horizontal Tabs Container with Side Arrow Hints & overflow control */}
            <div className="relative group w-full max-w-full overflow-hidden">
              {/* Left Scroll Button (visible on hover on desktop) */}
              <button
              ...
              <div 
                ref={headerScrollRef}
                className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar py-0.5 scroll-smooth w-full"
              >
              ...
              {/* Right Scroll Button (visible on hover on desktop) */}
              <button
